...
class ArenaStudyScreen extends ConsumerStatefulWidget {
  final List<TopicWithSubjectViewModel> dueTopics;
  
  const ArenaStudyScreen({super.key, required this.dueTopics});

  @override
  ConsumerState<ArenaStudyScreen> createState() => _ArenaStudyScreenState();
}

class _ArenaStudyScreenState extends ConsumerState<ArenaStudyScreen> {
  int _currentIndex = 0;
  int _correctCount = 0;
  int _againCount = 0;
  ...
  void initState() {
    super.initState();
    _stopwatch.start();
    // Shuffle topics so they are mixed across subjects
    widget.dueTopics.shuffle();
  }
  ...
  Future<void> _handleAnswer(bool isCorrect) async {
    if (_currentIndex >= widget.dueTopics.length) return;
    
    HapticFeedback.lightImpact();
    final entry = widget.dueTopics[_currentIndex];
    final isar = ref.read(isarProvider);
    
    final dbTopic = await isar.topics.get(entry.topic.id);
    if (dbTopic != null) {
      if (isCorrect) {
        _correctCount++;
      ...
      await notificationService.scheduleTopicReview(dbTopic, dbTopic.nextReviewDate!);
    }

    if (_currentIndex < widget.dueTopics.length - 1) {
      setState(() {
        _currentIndex++;
      });
    } else {
  ...
  Future<void> _finishSession() async {
    ...
    await isar.writeTxn(() async {
      ...
      if (log != null) {
        log.topicsReviewed += widget.dueTopics.length;
        await isar.activityLogs.put(log);
      } else {
        log = ActivityLog()
          ..date = today
          ..topicsReviewed = widget.dueTopics.length;
        await isar.activityLogs.put(log);
      }
    });
    ...
    await ref.read(notificationServiceProvider).scheduleDailyReviewNotification();
  ...
  Widget build(BuildContext context) {
      ...
      return Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              Icon(Icons.check_circle_outline_rounded, size: 64, color: AppTheme.primary),
              const SizedBox(height: 16),
              Text('All caught up!', style: AppTheme.displayLg()),
              const SizedBox(height: 8),
              Text('No topics due right now.', style: AppTheme.bodyMd()),
            ],
          ),
        ),
      );
    ...
    final currentEntry = widget.dueTopics[_currentIndex];
    final currentTopic = currentEntry.topic;
    final subject = currentEntry.subject;
    final progress = (_currentIndex + 1) / widget.dueTopics.length;
        ...
        title: Row(
          ...
          children: [
            ...
            Text(subject.name.toUpperCase(),
                style: AppTheme.labelSm(color: subject.accentColor)),
          ],
        ),
      ),
      body: SafeArea(
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: subject.accentColor.withValues(alpha: 0.3)),
                    boxShadow: [
                      BoxShadow(
                        color: subject.accentColor.withValues(alpha: 0.05),
                        blurRadius: 20,
                        spreadRadius: 2,
                      )
                    ],
